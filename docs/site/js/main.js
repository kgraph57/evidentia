/* Evidentia landing — copy buttons, optional chat player, CLI report demo. No dependencies. */
(function () {
  "use strict";
  ...
  function playCli(cli) {
    var lines = Array.prototype.slice.call(cli.querySelectorAll(".cli-line"));
    if (!lines.length) return;
    var replayBtn = cli.parentElement && cli.parentElement.querySelector(".cli-replay");
    var timers = [];
    var started = false;
    ...
    function play() {
      clearTimers();
      lines.forEach(function (el) { el.classList.remove("is-on"); });
      if (reduceMotion) {
  ...
  document.querySelectorAll("[data-cli]").forEach(playCli);
